...
function btnJoes(val,tipo,id,div=""){
    $.get("?action=ajaxJoes&t="+tipo+"&id="+id+"&l="+val,function(data){
        if ($.trim(data) != 'not_user' ) {
            let res = $.trim(data).split('|'); // res 0 1 núm likes dislikes, 2 3 liked disliked
            let l1 = res[2] == 1 ? `btn-primary">` : `" onclick="btnJoes(1,'${tipo}','${id}')">`;
            let l2 = res[3] == 1 ? `btn-danger">` : `" onclick="btnJoes(2,'${tipo}','${id}')">`;
            let html = `<label class="control-label btn ${l1} <svg  xmlns="http://www.w3.org/2000/svg"  width="24"  height="24"  viewBox="0 0 24 24"  fill="none"  stroke="currentColor"  stroke-width="2"  stroke-linecap="round"  stroke-linejoin="round"  class="icon icon-tabler icons-tabler-outline icon-tabler-thumb-up"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M7 11v8a1 1 0 0 1 -1 1h-2a1 1 0 0 1 -1 -1v-7a1 1 0 0 1 1 -1h3a4 4 0 0 0 4 -4v-1a2 2 0 0 1 4 0v5h3a2 2 0 0 1 2 2l-1 5a2 3 0 0 1 -2 2h-7a3 3 0 0 1 -3 -3" /></svg> ${res[0]} </label>
                <label class="control-label btn ${l2} <svg  xmlns="http://www.w3.org/2000/svg"  width="24"  height="24"  viewBox="0 0 24 24"  fill="none"  stroke="currentColor"  stroke-width="2"  stroke-linecap="round"  stroke-linejoin="round"  class="icon icon-tabler icons-tabler-outline icon-tabler-thumb-down"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M7 13v-8a1 1 0 0 0 -1 -1h-2a1 1 0 0 0 -1 1v7a1 1 0 0 0 1 1h3a4 4 0 0 1 4 4v1a2 2 0 0 0 4 0v-5h3a2 2 0 0 0 2 -2l-1 -5a2 3 0 0 0 -2 -2h-7a3 3 0 0 0 -3 3" /></svg> ${res[1]} </label>`;
            $("#joesDiv"+div).html(html);
        }
    })
};
...
function abrirArtigoSel(artigo = 0, iid = 0){
	if($("#artigo").val()>0) window.open("index.php?page=article&iid="+iid+"&id="+artigo);
}

function updateArtVinculado(tipo_dest = 'text', id_dest = '0', id_artigo = '0'){
	var dest = $("#artigo").val();
	$.get("?action=ajaxUpdateLinkArtigo&aid="+id_artigo+"&tipo="+tipo_dest+"&dest="+id_dest,function(data){
        if($.trim(data)=='ok'){
            //
        }else alert(data);
	})
